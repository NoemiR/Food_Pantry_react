...
const FamilyList = ({families, getFamily}) => {
	...
	//console.log(families, ' this is families')

	const familyList = families.map((family, i) => {
		return (
			<li className="familyList" key={family.id} id={family.id} onClick={getFamily}>{family.name} Family<br/>{family.phone}<br/>{family.address}</li>
		)
	})
